...
describe("Model Provider Configuration", () => {
  ...
  describe("Google Provider", () => {
    test("should have correct model mappings", () => {
      ...
      expect(googleModels[ModelClass.MEDIUM].name).toBe("gemini-2.0-flash-exp");
      expect(googleModels[ModelClass.LARGE].name).toBe("gemini-2.0-flash-exp");
    });
  });
  describe("Livepeer Provider", () => {
    test("should have correct endpoint configuration", () => {
      expect(getEndpoint(ModelProviderName.LIVEPEER)).toBe(
    ...
    test("should have correct model mappings", () => {
      ...
      expect(livepeerModels[ModelClass.SMALL]?.name).toBe(
        "meta-llama/Meta-Llama-3.1-8B-Instruct",
      );
      expect(livepeerModels[ModelClass.MEDIUM]?.name).toBe(
        "meta-llama/Meta-Llama-3.1-8B-Instruct",
      );
      expect(livepeerModels[ModelClass.LARGE]?.name).toBe(
        "meta-llama/Meta-Llama-3.1-8B-Instruct",
      );
      expect(livepeerModels[ModelClass.IMAGE]?.name).toBe(
...
describe("Model Retrieval Functions", () => {
  describe("getModel function", () => {
    test("should retrieve correct models for different providers and classes", () => {
      ...
      expect(
        models[ModelProviderName.ANTHROPIC].model[ModelClass.MEDIUM].name,
...
describe("Generation with Livepeer", () => {
  test("should have correct image generation settings", () => {
    ...
    expect(livepeerConfig.model[ModelClass.IMAGE]?.name).toBe(
      "ByteDance/SDXL-Lightning",
    );
    expect(
      getModelSettings(ModelProviderName.LIVEPEER, ModelClass.SMALL)
        ?.temperature,
    ).toBe(0);
  });
  ...
  test("should use default image model", () => {
    ...
    expect(getImageModelSettings(ModelProviderName.LIVEPEER)?.name).toBe(
      "ByteDance/SDXL-Lightning",
    );
  });
});
