...
import { KnowledgeScope } from "./types.ts";
  ...
  async cleanupDeletedKnowledgeFiles() {
    try {
      ...
      const parentDocuments = existingKnowledge.filter(
        (item) => !item.id.includes("chunk") && item.content.metadata?.source, // Must have a source path
      );
      ...
      for (const item of parentDocuments) {
        ...
        if (!existsSync(filePath)) {
          ...
          elizaLogger.debug(`[Cleanup] Using ID for removal: ${idToRemove}`);
