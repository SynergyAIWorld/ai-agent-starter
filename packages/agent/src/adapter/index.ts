...
import { DatabaseAdapter, elizaLogger, getEmbeddingConfig } from "../";
  ...
  implements IDatabaseCacheAdapter
{
  ...
  async createMemory(memory: Memory, tableName: string): Promise<void> {
    return this.withDatabase(async () => {
      ...
      await this.pool.query(
        `INSERT INTO memories (id, type, content, embedding, "userId", "roomId", "agentId", "unique",
                                       "createdAt")
  ...
  async getCachedEmbeddings(opts: {
    ...
    return this.withDatabase(async () => {
      try {
        ...
        const sql = `
                    WITH content_text AS (SELECT embedding,
                                                 COALESCE(
                                                         content ->>$2,
                                ''
                            ) as content_text
        ...
        try {
          ...
          await client.query(
            `INSERT INTO cache ("key", "agentId", "value", "createdAt")
                         VALUES ($1, $2, $3, CURRENT_TIMESTAMP) ON CONFLICT ("key", "agentId")
                         DO
                        UPDATE SET "value" = EXCLUDED.value, "createdAt" = CURRENT_TIMESTAMP`,
  ...
  async searchKnowledge(params: {
    ...
    return this.withDatabase(async () => {
      ...
      const sql = `
                WITH vector_scores AS (SELECT id,
                                              1 - (embedding <-> $1::vector) as vector_score
                                       FROM knowledge
                                       WHERE ("agentId" IS NULL AND "isShared" = true)
                                          OR "agentId" = $2
                                           AND embedding IS NOT NULL),
                     keyword_matches AS (SELECT id,
                                                CASE
    ...
    return this.withDatabase(async () => {
      ...
      try {
        ...
        if (metadata.isChunk && metadata.originalId) {
          ...
          await client.query(
            `
                            INSERT INTO knowledge (id, "agentId", content, embedding, "createdAt",
  ...
  private async createKnowledgeChunk(params: {
    ...
    await this.pool.query(
      `
                INSERT INTO knowledge (id, "agentId", content, embedding, "createdAt",
                                       "isMain", "originalId", "chunkIndex", "isShared")
                VALUES ($1, $2, $3, $4, to_timestamp($5 / 1000.0), $6, $7, $8, $9) ON CONFLICT (id) DO NOTHING
            `,
      [
