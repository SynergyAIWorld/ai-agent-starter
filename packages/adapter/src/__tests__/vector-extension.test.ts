...
describe("PostgresDatabaseAdapter - Vector Extension Validation", () => {
  ...
  const initializeDatabase = async (client: pg.PoolClient) => {
    ...
    try {
      ...
      const { rows: dimension } = await client.query(
        "SELECT get_embedding_dimension()",
      );
      elizaLogger.info("Vector dimension:", {
        dimension: dimension[0].get_embedding_dimension,
      });
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      elizaLogger.error(
        `Test environment setup failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  });
  ...
  afterEach(async () => {
    ...
    try {
      ...
      elizaLogger.error(
        `Test environment cleanup failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      throw error;
    }
  });
  ...
  describe("Schema and Extension Management", () => {
    test("should initialize with vector extension", async () => {
      ...
      try {
        ...
        elizaLogger.error(
          `Vector extension test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        throw error;
      }
    });
    ...
    test("should handle missing rooms table", async () => {
      ...
      try {
        ...
        elizaLogger.error(
          `Rooms table test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        throw error;
      }
    });
    ...
    test("should not reapply schema when everything exists", async () => {
      ...
      try {
        ...
        elizaLogger.error(
          `Schema reapplication test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        throw error;
      }
    });
    ...
    test("should handle transaction rollback on error", async () => {
      ...
      try {
        ...
        elizaLogger.error(
          `Transaction rollback test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        throw error;
      }
    });
  ...
  describe("Memory Operations with Vector", () => {
    ...
    test("should create and retrieve memory with vector embedding", async () => {
      ...
      const {
        rows: [{ get_embedding_dimension }],
      } = await testClient.query("SELECT get_embedding_dimension()");
      ...
      const parsedEmbedding =
        typeof retrieved?.embedding === "string"
          ? parseVectorString(retrieved.embedding)
          : retrieved?.embedding;
    ...
    test("should search memories by embedding", async () => {
      ...
      const parsedEmbedding =
        typeof results[0].embedding === "string"
          ? parseVectorString(results[0].embedding)
          : results[0].embedding;
      expect(parsedEmbedding).toHaveLength(1536);
    });
    ...
    test(
      ...
      async () => {
        ...
        try {
          await expect(
            adapter.createMemory(memory, TEST_TABLE),
          ).rejects.toThrow(
            "Invalid embedding dimension: expected 1536, got 100",
          );
          await testClient.query("ROLLBACK");
        } catch (error) {
          await testClient.query("ROLLBACK");
      ...
      { timeout: 30000 },
    ); // Increased timeout for retry attempts
  });
});
