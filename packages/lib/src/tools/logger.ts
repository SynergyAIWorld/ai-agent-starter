class W3Logger {
  ...
  #logWithStyle(
    ...
    if (strings.length > 1) {
      ...
      this.print(
        ...
        bg,
        strings.map((item) => {
          return `${this.useIcons ? `${icon} ` : ""}${String(item)}`;
        }),
      );
    }
  }
  ...
  debug(...strings: unknown[]) {
    if (!this.verbose) {
      ...
      // console.log(
      //     "[W3Logger] Debug message suppressed (verbose=false):",
      //     ...strings
      // );
      return;
    }
...
export const w3Logger = new W3Logger();
w3Logger.closeByNewLine = true;
w3Logger.useIcons = true;

export default w3Logger;
